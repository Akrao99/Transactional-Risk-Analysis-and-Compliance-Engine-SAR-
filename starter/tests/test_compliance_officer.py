...
```'''
        ...
        # Verify result (use type name to avoid dual-import class identity issues)
        assert type(result).__name__ == "ComplianceOfficerOutput"
    ...
    "completeness_check": true
        ...
        # Create minimal test data (CaseData requires at least one account)
        customer = CustomerData(
    ...
    def test_json_parsing_error(self):
        ...
        assert "parse" in logger.entries[0]["reasoning"] or "Narrative generation failed" in logger.entries[0]["reasoning"]
...
This completes the analysis.'''
    ...
    def test_api_call_parameters(self):
        ...
        mock_response = Mock()
        mock_response.choices = [Mock()]
        # Narrative must be >= 10 words when completeness_check is True (ComplianceOfficerOutput validation)
        mock_response.choices[0].message.content = '''{"narrative": "The customer conducted multiple suspicious transactions that may indicate structuring activity under BSA regulations.", "narrative_reasoning": "Test", "regulatory_citations": ["31 CFR 1020.320"], "completeness_check": true}'''
        mock_client.chat.completions.create.return_value = mock_response
        
        logger = ExplainabilityLogger("test_api_compliance.jsonl")
        agent = ComplianceOfficerAgent(mock_client, logger, model="gpt-3.5-turbo")
        
        # Create minimal case and risk analysis (CaseData requires at least one account)
        customer = CustomerData(
